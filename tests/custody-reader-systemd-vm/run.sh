...
machine=virt-7.2
data_path=()
if test "${1:-adapter}" = loader-kvm-modern || test "${1:-adapter}" = joined-kvm-modern || test "${1:-adapter}" = garage-kvm-modern; then
    ...
    if test "${1:-adapter}" = loader-kvm-modern; then
        ...
        # Actual Garage diagnostic source45a0a1d2, closed client/provider categories.
        test "$(sha256sum /adapter | awk '{print $1}')" = def4e8538088dee72c6faf6b106564ab062f0001f4a969c49979093926b9410e
    fi
fi
...
if test "${1:-adapter}" != loader-kvm-modern && test "${1:-adapter}" != joined-kvm-modern && test "${1:-adapter}" != garage-kvm-modern; then strip --strip-debug /tmp/seed/adapter; fi
...
case "${1:-adapter}" in
 ...
 joined-kvm-modern|garage-kvm-modern)
   ...
   if test "${1:-adapter}" = garage-kvm-modern; then
