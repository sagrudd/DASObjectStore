...
finish() {
    ...
    if test "$status" != 0; then
        ...
        for diagnostic in tls-prepare tls-reader tls-verifier; do
...
for mode in "${modes[@]}"; do
    phase="prepare_directories_$mode"
    ...
    phase="unit_setup_$mode"
    install -m 600 /dev/null /run/das-systemd-vm-fixture/tls-reader.log
    install -m 600 /dev/null /run/das-systemd-vm-fixture/tls-verifier.log
...
StandardOutput=append:/run/das-systemd-vm-fixture/tls-reader.log
StandardError=append:/run/das-systemd-vm-fixture/tls-reader.log
UNIT
...
StandardOutput=append:/run/das-systemd-vm-fixture/tls-verifier.log
StandardError=append:/run/das-systemd-vm-fixture/tls-verifier.log
UNIT
    ...
    phase="lifecycle_setup_$lifecycle"
    ...
    for unused in $(seq 1 750); do
        ...
        if systemctl is-failed --quiet das-vm-tls-reader.service; then failure_line=$LINENO; exit 1; fi
        if test "$garage_existing" = no && systemctl is-failed --quiet das-vm-protocol.service; then failure_line=$LINENO; exit 1; fi
        sleep .2
    done
    test "$ready" = yes
    phase="verifier_start_$lifecycle"
    systemctl start das-vm-tls-verifier.service
    phase="verifier_terminal_$lifecycle"
    wait_success das-vm-tls-verifier.service /run/das-vm-verifier-material/passed
    phase="reader_terminal_$lifecycle"
    ...
    phase="publication_journal_$lifecycle"
    expected=1
