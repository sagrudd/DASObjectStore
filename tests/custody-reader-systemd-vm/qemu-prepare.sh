#!/bin/bash
# Public download only. An independent controller enforces 900s and cleanup.
# No package installation, guest boot, devices, host mounts, or private inputs.
set -euo pipefail
export LC_ALL=C
umask 022
trap 'exit 124' TERM
...
# Retain exact URLs. The later separately reviewed offline transaction must not
# resolve a newer repository view or use updates/testing/weak dependencies.
dnf5 --quiet --releasever=44 --setopt=retries=0 --setopt=timeout=60 \
 ...
 download --resolve --alldeps --arch=aarch64 --arch=noarch --url --urlprotocol=https \
 qemu-system-aarch64-core qemu-img edk2-aarch64 genisoimage binutils createrepo_c \
...
while IFS= read -r url; do
    ...
    curl --silent --show-error --fail --location --proto '=https' --proto-redir '=https' \
      --connect-timeout 15 --max-time 60 --retry 0 --max-filesize "$remaining" \
    ...
    rpmkeys --define '_keyring rpmdb' --define '_pkgverify_level signature' \
      --dbpath "$out/keydb" --checksig --verbose "$destination" >> "$out/evidence/signatures.txt"
...
test "$count" -gt 0
for package in qemu-system-aarch64-core qemu-img edk2-aarch64 genisoimage binutils createrepo_c; do
    grep -q "^$package " "$out/evidence/nevra.txt"
done
...
echo PUBLIC_FEDORA_QEMU_DOWNLOAD_COMPLETE
