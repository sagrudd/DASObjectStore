...
| --- | --- | --- | --- | --- |
| `../prosopikon` | Identity contracts and Yew components. Pinned by `Cargo.toml`; package preflight is `packaging/pinned-mnemosyne-package-sources.sh`. | `446c9a359eea1ffa2ca4f53b4dd2bdf950b610e2` (`main`, Prosopikon 0.23.0 governed installation-owner DAS entitlement) | `https://github.com/sagrudd/prosopikon.git` | Primary checkout state is not used for release packaging; package builds require a clean checkout at the pinned revision. Monas and DASObjectStore retain this exact actor-type source revision so the signed first-installation owner's typed Administer entitlement crosses the embedded boundary without translation. |
