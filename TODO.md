...
- [~] Implement profile-aware browse, download, verify, capacity, health,
  ...
  - [x] Prevent generic appliance ``store drain`` from acting on profile-bound
    stores and route confirmed ``store delete`` through daemon-owned,
...
- [x] Make first-use remote TLS enrollment a single-command, default-deny
  ...
  - [x] Separate stable appliance identity from certificate generations; verify
    domain-cert CA chain, server EKU, validity, DNS/IP SAN and authenticated
    appliance-ID continuity before automatic leaf/SPKI renewal. Add
  ...
  - [x] Add the integrated ``dasobjectstore-remote resync`` workflow with a
    versioned capability/build descriptor, protocol negotiation, secret-free
...
- [x] Add daemon-owned ``disk_housekeeping``: continuous-idle admission,
  draining/suspect evacuation, fractional-capacity rebalancing, nine-week
  verified placement rotation (or fail-closed rehash where no target exists),
  a hard ten-percent configured I/O budget, periodic evidence-based SSD
  cleanup, and a physical SSD-residency report separating settled residue,
  pending HDD work, orphaned data, provider data, and unexplained bytes.
