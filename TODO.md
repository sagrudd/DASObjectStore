...
- [ ] Model direct-copy durability finalization explicitly: after bytes reach the destination, emit distinct `fsync` and atomic-rename/metadata-commit states rather than reporting a transfer as a vague `settling` wait. Record and display their durations separately from byte-copy throughput.
...
- [x] Decouple daemon progress/socket reporting from the I/O hot path with byte/time coalescing that preserves phase and target-assignment transitions. TUI redraw cadence remains tracked with the live operator telemetry presentation work.
