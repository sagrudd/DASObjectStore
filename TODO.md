...
- [ ] Extend daemon ingest events with target-disk assignment before copying begins, bytes written, and complete queue/worker state. Keep the phase-rate fields above and use short-window rates so stalled transfers are distinguishable from active ones.
