...
- [ ] Complete queue/worker event coverage for scan, source-read, SSD-stage, HDD-write, verification, and finalization so every progress frame reports each queue and worker lane rather than only inferred source/HDD depths.
