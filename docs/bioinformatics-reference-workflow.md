...
Large server-local datasets may use a CLI-managed direct-to-HDD path. The
command mirrors `ingest files` and accepts a folder tree; it differs by
bypassing SSD staging and landing the imported files directly onto managed HDDs.

```bash
dasobjectstore ingest direct-import ensembl-release-113 \
  --source /downloads/homo_sapiens/release-113 \
  --object-type ena_sra \
  --hdd-workers 5 \
  --strict
```

The direct path computes hashes during the daemon-managed copy process and
settles policy-satisfying HDD copies without first writing payloads to SSD.
Metadata commit and object-service exposure can then build
