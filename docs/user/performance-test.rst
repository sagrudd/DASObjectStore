...
* SSD-first pipeline throughput while source files continue landing on SSD
...
``ssd-pipeline`` stages to SSD while FIFO HDD drain workers consume the queue;
if HDD drain lags, SSD backlog can grow toward the selected workload size.
