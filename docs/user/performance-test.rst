...
Packaged appliance installations ship a DASObjectStore-owned compatibility
wrapper at ``/usr/libexec/dasobjectstore/gnostikon-workflow-control`` and
declare a Docker-compatible container runtime as a runtime dependency because
formal PDF reporting is a supported operator surface. If the container runtime
...
Formal PDF rendering requires the packaged DASObjectStore
``gnostikon-workflow-control`` compatibility wrapper, or an external
``gnostikon-workflow-control`` command in development builds, with access to the
...
``gnostikon-workflow-control`` sibling repository for local development and
upstream parity. Package builds install the DASObjectStore-owned wrapper and
declare the container runtime as a runtime dependency so a freshly installed
appliance cannot silently expose a broken report rebuild button.
