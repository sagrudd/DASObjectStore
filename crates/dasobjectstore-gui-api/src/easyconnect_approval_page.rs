...
pub(crate) async fn easyconnect_browser_approval(
    ...
    let policy = format!(
        "default-src 'none'; script-src 'nonce-{nonce}'; style-src 'nonce-{nonce}'; img-src 'self'; connect-src 'self'; form-action http://127.0.0.1:*; base-uri 'none'; frame-ancestors 'none'"
    );
    headers.insert(
