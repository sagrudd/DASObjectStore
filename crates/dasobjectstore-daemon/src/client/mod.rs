    ...
    T: DaemonClientTransport,
{
    ...
    ) -> Result<ProfileCatalogueExportResponse, DaemonClientError> {
        match self.send(DaemonApiRequest::ProfileCatalogueExport(request))? {
            DaemonApiResponse::ProfileCatalogueExport(response) => Ok(response),
            response => Err(unexpected("profile_catalogue_export", response)),
        }
    }

    pub fn profile_catalogue_import(
        &self,
        request: ProfileCatalogueImportRequest,
    ) -> Result<ProfileCatalogueImportResponse, DaemonClientError> {
    ...
    ) -> Result<ProfileDiagnosticsResponse, DaemonClientError> {
        match self.send(DaemonApiRequest::ProfileDiagnostics(request))? {
            DaemonApiResponse::ProfileDiagnostics(response) => Ok(response),
            response => Err(unexpected("profile_diagnostics", response)),
        }
    }
    ...
    ) -> Result<ApplicationKeyRegistrationResponse, DaemonClientError> {
        match self.send(DaemonApiRequest::RegisterApplicationKey(request))? {
            DaemonApiResponse::RegisterApplicationKey(response) => Ok(response),
            response => Err(unexpected("register_application_key", response)),
        }
    }
    ...
    ) -> Result<ApplicationCredentialRevocationResponse, DaemonClientError> {
        match self.send(DaemonApiRequest::RevokeApplicationCredential(request))? {
            DaemonApiResponse::RevokeApplicationCredential(response) => Ok(response),
            response => Err(unexpected("revoke_application_credential", response)),
        }
    }
...
fn response_name(response: &DaemonApiResponse) -> &'static str {
    match response {
        ...
        DaemonApiResponse::ProviderStreamUploadReady(_) => "provider_stream_upload_ready",
...
mod tests {
    ...
    use crate::api::{
        ...
        ApplicationMtlsAuthorizationResponse, AssignLocalUserToLocalGroupRequest,
        AssignLocalUserToLocalGroupResponse, CapacityAdmissionRequest, CapacityAdmissionResponse,
        ...
        DaemonSsdPressure, DiskLockdownRequest, DiskLockdownResponse, ObjectBrowserPageRequest,
        ...
        PrepareEnclosureRequest, PrepareEnclosureResponse, ProfileBrowserRequest,
