//! Linux appliance telemetry collectors for daemon-owned dashboard history.
...
pub fn collect_linux_cpu_telemetry(
    ...
    let (usage_percent, missing_reason) = match previous {
        None => (None, Some("initial_cpu_snapshot".to_string())),
        Some(previous) => {
            ...
            {
                (None, Some("cpu_counter_reset".to_string()))
            } else if total_delta == 0 || idle_delta > total_delta {
                (None, Some("cpu_counter_delta_unavailable".to_string()))
            } else {
...
pub fn collect_linux_memory_telemetry(proc_meminfo: &str) -> ApplianceMemoryTelemetry {
    ...
    let missing_reason = if total_bytes.is_none() {
        Some("memtotal_missing".to_string())
    } else if available_bytes.is_none() {
        Some("memavailable_missing".to_string())
    } else {
        None
    };
