...
use crate::runtime::{
    ...
    remote_object_group_status, remote_object_snapshot, remove_profile_binding_if_matches,
    resolve_mtls_application_identity_by_fingerprint, resolve_object_download_with_hdd_root,
    resolve_object_folder_download_with_hdd_root, restore_profile_binding_if_matches,
    upsert_application_identity, upsert_application_key, upsert_endpoint_inventory_record,
    upsert_profile_binding, validate_application_key_enrollment, validate_profile_binding_claim,
...
use self::errors::{
    ApplianceTelemetryAccessFailure, IngestAuthorizationFailure, ObjectBrowserAccessFailure,
    RemoteEasyconnectExchangeDispatchError, RemoteEasyconnectRenewalDispatchError,
    RemoteEasyconnectStoreInventoryError,
};
...
mod tests {
    ...
    fn folder_profile_create_bootstraps_private_namespace_without_adopting_files() {
        ...
        let snapshot: serde_json::Value =
            serde_json::from_slice(&first_catalogue).expect("catalogue JSON");
        assert_eq!(snapshot["schema_version"], 1);
        assert_eq!(snapshot["store_id"], "folder-create");
        ...
        assert_eq!(
            fs::read(&catalogue_path).expect("catalogue reread"),
            first_catalogue
        );
        assert_eq!(fs::read(&user_file).expect("user file read"), b"user-owned");
    ...
    #[test]
    fn rejects_destructive_ingest_queue_drain_without_authenticated_administrator() {
        let handler =
            DaemonRequestHandler::new(FakeService::default(), FixedDaemonClock::new("now"));
        let response = handler
            .handle(DaemonApiRequest::IngestQueueDrain(
    ...
    fn application_identity_registration_request_for_auth_test(
        application_id: &str,
    ...
    fn application_key_registration_request_for_auth_test(
        application_id: &str,
        key_id: &str,
    ...
    #[test]
    fn application_identity_registration_requires_authenticated_administrator() {
        let root = temp_root("application-identity-auth");
        let registry = root.join("application-identities.json");
        let handler = DaemonRequestHandler::new(
            FakeService::default(),
        ...
        let unauthenticated = handler
            .handle(DaemonApiRequest::RegisterApplicationIdentity(
                request.clone(),
            ))
            .expect("request handled");
        assert!(matches!(
    ...
    #[test]
    fn application_identity_registration_persists_policy_and_redacts_request_actor() {
        let root = temp_root("application-identity-admin");
        let registry = root.join("application-identities.json");
        let handler = DaemonRequestHandler::new(
            FakeService::default(),
    ...
    #[test]
    fn application_key_registration_requires_authenticated_administrator() {
        let root = temp_root("application-key-auth");
        ...
        let response = handler
            .handle_with_progress_for_actor(
                DaemonApiRequest::RegisterApplicationKey(
                    application_key_registration_request_for_auth_test("admin", "key-1"),
                ),
                Some(&actor),
                |_| Ok(()),
            )
    ...
    fn monas_peer_reads_only_the_phoreus_profile_readiness_gate() {
        ...
        cleanup(&root);
        let (store_registry, subobject_registry) =
            write_test_store_registry(&root, "phoreus", None);
        let handler = DaemonRequestHandler::new(
            FakeService::default(),
            FixedDaemonClock::new("2026-07-13T11:05:00Z"),
        )
