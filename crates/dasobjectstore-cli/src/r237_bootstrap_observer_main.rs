...
mod tests {
    ...
    fn standalone_binary_has_no_argument_or_apply_contract() {
        ...
        let production_source = source
            ...
            .0;
        assert!(production_source.contains("args_os().len() != 1"));
        assert!(!production_source.contains("--target"));
        assert!(!production_source.contains("--apply"));
        assert!(!production_source.contains("systemctl"));
    }
}
