...
use crate::linux_smart::{parse_smartctl_json, smartctl_health_args};
...
/// Safe path-walk: each parent is opened with `O_PATH | O_NOFOLLOW`, which
/// obtains only a descriptor and does not read directory contents or update
/// directory atime. The final regular file is opened with
/// `O_NOATIME | O_NOFOLLOW`. There is no ordinary-read fallback, so a symlink,
/// FIFO, device, directory, replacement, or missing protected ancestor
/// produces no trusted observation.
#[cfg(target_os = "linux")]
...
mod tests {
    ...
    fn observer_source_has_no_mutating_or_transport_syscall_surface() {
        ...
        assert!(production_source.contains("libc::O_NOATIME"));
        assert!(production_source.contains("libc::O_NOFOLLOW"));
        assert!(production_source.contains("libc::O_PATH"));
        assert!(production_source.contains("libc::fstatat"));
    }
