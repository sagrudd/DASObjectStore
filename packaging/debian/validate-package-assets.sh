...
require_absent "$web_service" "RuntimeDirectory=dasobjectstore"
require_absent "$web_service" "StateDirectory=dasobjectstore"
require_absent "$s3_gateway_service" "RuntimeDirectory=dasobjectstore"
require_absent "$s3_gateway_service" "StateDirectory=dasobjectstore"
...
require_text "$authority_retirement_service" "NoNewPrivileges=false"
require_absent "$authority_retirement_service" "NoNewPrivileges=yes"
require_text "$s3_gateway_service" "Requires=dasobjectstored.service"
require_text "$s3_gateway_service" "NoNewPrivileges=true"
require_text "$s3_gateway_service" "ExecStart=/usr/bin/dasobjectstore-s3-gateway --config /etc/dasobjectstore/s3-gateway.json"
require_absent "$s3_gateway_service" "[Install]"
...
require_text "$build_deb" '"$cargo_target_dir/release/dasobjectstore-r237-bootstrap-observer"'
...
require_text "$build_deb" 'usr/libexec/dasobjectstore/dasobjectstore-authority-retirement-finalize'
...
require_text "$build_deb" 'Depends: ca-certificates, openssh-client, acl, mergerfs, nfs-kernel-server, python3, quota, smartmontools, udisks2, docker.io | docker-ce, docker-buildx | docker-buildx-plugin'
require_text "$build_deb" 'etc/dasobjectstore-remote/site-trust-sources.d'
...
require_text "$build_rpm" '"$cargo_target_dir/release/dasobjectstore-r237-bootstrap-observer"'
for builder in "$build_deb" "$build_rpm"; do
  require_absent "$builder" 'dasobjectstore-r237-bootstrap-observer.service'
  require_absent "$builder" 'dasobjectstore-r237-bootstrap-observer.socket'
  require_absent "$builder" 'dasobjectstore-r237-bootstrap-observer.json'
  require_absent "$builder" 'dasobjectstore-r237-bootstrap-observer.conf'
  require_absent "$builder" 'sudo dasobjectstore-r237-bootstrap-observer'
  require_absent "$builder" 'mnemosyne-r237-custody-marker'
  require_absent "$builder" 'r237-bootstrap-observer-credential'
done
...
require_text "$build_deb" '"$cargo_target_dir/release/dasobjectstore-workspace-host"'
...
require_text "$build_rpm" '/usr/libexec/dasobjectstore/dasobjectstore-authority-retirement-finalize'
...
require_text "$build_rpm" '"$cargo_target_dir/release/dasobjectstored"'
require_text "$build_rpm" '"$cargo_target_dir/release/dasobjectstore-remote"'
...
require_text "$build_rpm" 'Requires:       openssh-clients'
require_text "$build_rpm" '/etc/dasobjectstore-remote/site-trust-sources.d'
...
require_text "$build_remote_deb" 'etc/dasobjectstore-remote/site-trust-sources.d'
require_text "$build_remote_deb" 'Depends: ca-certificates, openssh-client'
...
require_text "$build_remote_rpm" 'docs/contracts/remote-site-trust-provisioning-v1.md'
require_text "$build_remote_rpm" 'docs/schemas/dasobjectstore.remote-site-trust-source.v1.schema.json'
require_text "$build_remote_rpm" '/etc/dasobjectstore-remote/site-trust-sources.d'
require_text "$build_remote_rpm" 'Requires:       openssh-clients'
