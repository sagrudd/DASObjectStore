...
install -m 0755 "$packaging_linux/usr/libexec/dasobjectstore/prepare-monas-managed-root-traversal" \
  "$payload_root/usr/libexec/dasobjectstore/prepare-monas-managed-root-traversal"
...
install -m 0644 "$packaging_product/plugin-process-descriptor.json" \
  "$payload_root/opt/dasobjectstore/plugin-process-descriptor.json"
...
if [ -e /var/lib/dasobjectstore/appliance-identity.json ] || [ -L /var/lib/dasobjectstore/appliance-identity.json ]; then
  /usr/libexec/dasobjectstore/manage-monas-access-boundary publish-identity
fi
