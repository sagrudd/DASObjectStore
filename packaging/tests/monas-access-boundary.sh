...
case "$path" in */stores.json) mode=640;; */dasobjectstored.sock) mode=660;; */auth|*/srv/dasobjectstore|*/tls) group=dasobjectstore;; esac
...
case "$format" in %U) echo "$owner";; %G) echo "$group";; %a) echo "$mode";; *) exit 64;; esac
EOF
cat >"$bin/ss" <<'EOF'
#!/bin/sh
...
cat >"$bin/chgrp" <<'EOF'
#!/bin/sh
[ "$1" = mnemosyne-pistis-das ] && [ "$2" = "$TEST_SOCKET" ]
EOF
cat >"$bin/unlink" <<'EOF'
#!/bin/sh
...
EOF
chmod 0755 "$bin"/*
run() { TEST_STATE="$state" TEST_SOCKET="$root/run/dasobjectstore/dasobjectstored.sock" PATH="$bin:$PATH" "$helper" --test-root "$root" "$1"; }
