...
- Advance both Prosopikon workspace dependencies to immutable revision
  ``0a2f82b4938bdcfde61a33aeb89ba62bf3ab3ffa`` (Prosopikon #49). This is a
  dependency lockstep only: it adopts the shared typed Site Trust endpoint
  identity contract without changing DASObjectStore adapters, actor policy,
  authentication behaviour, package lifecycle, or host state. The version is
  deliberately above the already-installed 0.161.1 appliance package; package
  builders must never produce a formal downgrade.
...
- Correct the Jenkins dossier EvidenceRef contract: ``subject_digest`` binds
  the versioned, domain-separated Jenkins dossier subject projection, not the
  final dossier digest that contains the EvidenceRef itself. The final
  canonical dossier digest remains reference-complete and approval-bound.
...
- Add the owner-side canonical projection for a committed `jenkins.dossier`
  ...
  the domain-separated Expedition dossier subject digest without carrying
  authority or secret material. Live Monas/Pistis grant admission, daemon
  persistence, independent read-back, and retained Jenkins qualification
...
- Migrate the host-composed ObjectStore object-download route to the verified
  Pistis browser envelope. The daemon now accepts that envelope only from the
  fixed `dasobjectstore` Unix service peer and revalidates its exact endpoint
  and canonical object scope before serving data. This host route has no
  delegated-OS or provider-stream fallback.
...
- Reconcile ADR-0005's reviewed cross-project revisions with the permanent
...
- Add a provider-neutral profile S3 PUT adapter with known-length quota
  reservations, in-flight checksums, durable finalization, and catalogue
  commit semantics.
...
- Add authenticated redacted profile inspection for folder drift and missing
  roots without exposing daemon-owned paths or recreating storage namespaces.
...
- Preflight profile-binding root and identity claims before initializing a
  capacity ledger, preventing rejected registrations from leaving orphaned
...
- Publish an explicitly supplied daemon-local store definition only after
  profile binding validation and capacity-ledger initialization succeed.
...
- Add the `store profile-binding` CLI workflow for validated daemon-owned
  create/adopt binding registration from a portable manifest.
...
- Add a state-scoped profile-binding registry path helper with an explicit
  environment override, keeping daemon and container deployments consistent.
...
- Encode the approved storage-authority and consumer-boundary decisions and
  add a bounded macOS validation-root mode to the local Docker profile.
...
- Add focused Web source/component contracts for Mnemosyne footer mounting,
  TaskPane keyboard/focus boundaries, Local Access, endpoint editing, and
  target-scoped Remote Upload gating.
...
- Commit verified migration destinations through the authority before advancing
...
- Add the profile-neutral `ObjectCatalogueAuthority` batch contract and adapt
  the durable folder catalogue without changing portable manifest v1.
...
- Add a bounded, profile-neutral folder catalogue browser projection that
  preserves authoritative keys, versions, sizes, checksums, and locations
  without inventing appliance-only placement metadata.

## 0.72.87 - 2026-07-13

- Add a compact ``ingest control --tui`` acknowledgement with typed state,
  preview warnings, and parser/renderer regression coverage.

## 0.72.86 - 2026-07-13

- Document operator triage for SSD/HDD ingest pressure and safe daemon control
  actions, including escalation evidence and blocked soak boundaries.
...
- Add an authenticated store-scoped Web capacity-status route through the
  bounded daemon bridge with typed saturation, degradation, and deadline
  failures.
...
- Make Local Access users-first with per-user qualification/group authority,
  an Add-user task pane, secondary group management, and responsive styles.
